<#-- Tidepool override of keycloak.v2/login/field.ftl. The only differences vs -->
<#-- upstream are the macro defaults: required=true everywhere. This inverts -->
<#-- the convention so optional fields must opt in (required=false), avoiding -->
<#-- the need to mark every login field "required" just to suppress the -->
<#-- "(optional)" suffix our CSS injects on optional labels. -->
<#macro group name label error="" required=true>
...
<#macro input name label value="" required=true autocomplete="off" fieldName=name error=messagesPerField.get(fieldName) autofocus=false>
...
<#macro password name label value="" required=true forgotPassword=false fieldName=name error=messagesPerField.get(fieldName) autocomplete="off" autofocus=false>
  <@group name=name label=label error=error required=required>
